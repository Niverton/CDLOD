...
d'une sphère en vue d'une application en temps intéractif, afin
...
une planète.\\
Cette partie ne sera pas implémenté, elle sera utilisé grâce à une bibliothèque de création de carte de hauteur.
...
performance imposées par le client. Nous pouvons cependant estimer les
performances de notre système en regardant des algorithmes similaires,
comme par exemple \cite{PlanetRenderer}. L'algorithme étant implémenté
sur GPU, notre implémentation CPU ne sera pas aussi performante.
Néanmoins nous nous attendons à des performances résonnables de l'ordre
de plusieurs centaines d'images par secondes sur un ordinateur récent.
%TODO Si possible intégrer capture d'écran de PlanetRenderer avec
%compteur de FPS bien visible
...
Lors de la mise à jours du terrain, des nouveaux sommets sont créés et sont rattaché 
à ces voisins de sorte a former un triangle. L'implémentation ne doit par permettre à 
un sommet de se retrouver seul, c'est à dire éviter la formation de trous dans le maillage.\\
Elle doit aussi éviter de former des triangles qui s'intersectent.
Pour cela, pour un sommet dans un maillage, ce sommet sera relié uniquement a ces plus
 proches voisins
