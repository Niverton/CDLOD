...
HeightmapCache::HeightmapCache(int cacheSize, int textureResolution)
    : mCacheSize(cacheSize), mTexResolution(textureResolution) {}
...
void HeightmapCache::Initialize() {
  unsigned long long int placeholderId =
  ...
  for (int i = 0; i < mCacheSize; i++) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, mTexResolution, mTexResolution,
  ...
  std::cout << "Height map cache allocated. Total size: "
            << ((float)(mCacheSize * mTexResolution * mTexResolution * 4.0f *
                        4.0f) /
                (1024.0f * 1024.0f))
            << "MB" << std::endl;
}

bool HeightmapCache::hasActiveTexture(const unsigned long long int &id) {
  return mActiveTextures.find(id) != mActiveTextures.end();
}

bool HeightmapCache::hasInactiveTexture(const unsigned long long int &id) {
  auto freeEnd = mFreeTextures.end();
  for (auto it = mFreeTextures.begin(); it != freeEnd; ++it)
    if ((*it).ID == id)
      return true;
...
bool HeightmapCache::unfreeInactiveTexture(const unsigned long long int &id) {
  auto freeEnd = mFreeTextures.end();
  for (auto it = mFreeTextures.begin(); it != freeEnd; ++it) {
    if ((*it).ID == id) {
...
bool HeightmapCache::hasFreeTexture() { return mFreeTextures.size() > 0; }

Texture *HeightmapCache::getFreeTexture(const unsigned long long int &newId) {
...
Texture *HeightmapCache::getTexture(const unsigned long long int &id,
                                    bool searchFreeTextures) {
...
void HeightmapCache::clearLastFetched() { mLastFetched.clear(); }

void HeightmapCache::freeTexture() {
  bool freed = false;

  while (!freed && mLastFetched.size() > 0) {
    unsigned long long int id = mLastFetched.back();
  ...
  if (!freed) {
    ...
    std::cout << __FILE__ << " should have not reached this point " << __LINE__
              << std::endl;
  }
}

bool HeightmapCache::freeTexture(const unsigned long long int &id) {
