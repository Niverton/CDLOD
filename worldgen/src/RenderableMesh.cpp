...
RenderableMesh::RenderableMesh()
    : vertexArray(0), indexBuffer(0), vertexBuffer(0), mpDiffuseTexture(NULL),
      mpBumpMap(NULL), mpSpecularMap(NULL), mpMaterial(NULL),
      mpEmissiveMap(NULL) {}
...
void RenderableMesh::Initialize(const ColoredMesh &mesh) {
  ...
  glVertexAttribPointer(2, 4, GL_FLOAT, GL_TRUE, sizeof(ColoredVertex),
                        (char *)NULL + 24);

  glGenBuffers(1, &vertexBuffer);
  glBindBuffer(GL_ARRAY_BUFFER, vertexBuffer);
  glBufferData(GL_ARRAY_BUFFER, sizeof(ColoredVertex) * mesh.Vertices.size(),
               &mesh.Vertices[0], GL_STATIC_DRAW);
