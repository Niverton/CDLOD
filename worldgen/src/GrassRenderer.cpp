...
GrassRenderer::GrassRenderer()
    : mpGrassPatch(NULL), mpGrassProgram(0), mpGrassTexture(NULL),
      mTimeUniformLocation(0), mCurrentTime(0.0f) {
...
void GrassRenderer::Initialize() {
  ...
  GLuint perFrameUniformIndex =
      glGetUniformBlockIndex(mpGrassProgram, "UniformPerFrameBlock");
  glUniformBlockBinding(mpGrassProgram, perFrameUniformIndex, 1);

  GLuint perObjectUniformIndex =
      glGetUniformBlockIndex(mpGrassProgram, "UniformPerObjectBlock");
...
void GrassRenderer::Update(float dt) { mCurrentTime += dt; }

void GrassRenderer::Render(const PerFrameBlock &perFrame,
