...
TerrainMesh::TerrainMesh(int resolution, float scale)
    : mResolution(resolution), mScale(scale), mIndexCount(0),
...
void TerrainMesh::Initialize() {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(GLuint) * indices[i].size(),
...
void TerrainMesh::generateMesh(int resolution, float scale,
                               vector<TerrainVertex> &vertexOut,
                               vector<vector<GLuint>> &indexOut) {
  vertexOut.clear();
  resolution++;
  ...
  for (int x = 0; x < resolution; x++) {
    ...
    for (int z = 0; z < resolution; z++) {
      ...
      vertexOut.push_back(TerrainVertex(
          vec3(currentX, 0.5f * scale, -halfDepth + z * dz), vec2(texX, texY)));
    }
  }
