...
OceanPlane::OceanPlane(Camera *baseCamera)
    : mpVertexBuffer(0), mpIndexBuffer(0), mpRenderProgram(0),
      mpCamera(baseCamera) {
...
void OceanPlane::Initialize() {
  ShaderInfo shaderInfo[] = {{GL_VERTEX_SHADER, "Shaders/ocean_vs.glsl"},
  ...
  GLuint perFrameUniformIndex =
      glGetUniformBlockIndex(mpRenderProgram, "UniformPerFrameBlock");
  glUniformBlockBinding(mpRenderProgram, perFrameUniformIndex, 1);

  GLuint perObjectUniformIndex =
      glGetUniformBlockIndex(mpRenderProgram, "UniformPerObjectBlock");
  glUniformBlockBinding(mpRenderProgram, perObjectUniformIndex, 2);

  GLuint oceanUniformIndex =
      glGetUniformBlockIndex(mpRenderProgram, "UniformOceanBlock");
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(vec3) * vertices.size(), &vertices[0],
               GL_STATIC_DRAW);

  glBindVertexArray(0);
}

void OceanPlane::Render(const PerFrameBlock &perFrame,
  ...
  glBufferData(GL_UNIFORM_BUFFER, sizeof(OceanUniformBlock), &oceanBlock,
               GL_DYNAMIC_DRAW);
...
void OceanPlane::generatePlane(int c, int r, vector<vec3> &vertices,
                               vector<GLuint> &indices) {
  vertices.clear();
  indices.clear();
...
void OceanPlane::transformPoint(vec3 &point, const mat4 &inverseProjection,
  ...
  bool intersected =
      intersectRaySphere(vec3(mpCamera->getPosition()), direction, planetCenter,
                         6378.1 * 1000.0, t, point);
...
bool OceanPlane::intersectRaySphere(const vec3 &origin, const vec3 &direction,
