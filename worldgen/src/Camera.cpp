...
Camera::Camera(GLFWwindow *window, const vec3 &position, const vec3 &direction,
               const vec3 &up)
    : mpWindow(window), mPosition(position), mUp(up), mRotation(),
      mVelocity(100.0f), mPitch(0.0f), mYaw(0.0f), mReadingMouse(false),
      mDirectionChanged(true), mFreeRoam(true) {
  mNear = 0.1f;
  mFar = 300000000.0f;
  setProjection(1280, 720, 45.0f);
...
// When very large values were used as the camera position the looking direction
// controls became
//  fairly broken because of the vector subtraction that was in the glm::lookat
//  function
...
void Camera::OnMouseMove(float x, float y) {
  if (mReadingMouse) {
    ...
    // mRotation = glm::rotate(mRotation, dx * 100.0f, mRotation *
    // vec3(0.0f, 1.0f, 0.0f));  mRotation = glm::rotate(mRotation, dy * 100.0f,
    // mRotation * vec3(0.0f, 0.0f, 1.0f));
...
void Camera::setProjection(int width, int height, float fovVertical) {
  mProjection =
      glm::perspectiveFov(45.0f, (float)width, (float)height, mNear, mFar);
}
