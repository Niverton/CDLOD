...
void CDDSImage::create_textureCubemap(
    unsigned int format, unsigned int components, const CTexture &positiveX,
    const CTexture &negativeX, const CTexture &positiveY,
    const CTexture &negativeY, const CTexture &positiveZ,
...
bool CDDSImage::load(string filename, bool flipImage) {
  ...
  for (unsigned int n = 0; n < (unsigned int)(m_type == TextureCubemap ? 6 : 1);
       n++) {
...
bool CDDSImage::upload_texture1D() {
  ...
  if (is_compressed()) {
    ...
    glCompressedTexImage1DARB(GL_TEXTURE_1D, 0, m_format, baseImage.get_width(),
    ...
    for (unsigned int i = 0; i < baseImage.get_num_mipmaps(); i++) {
      ...
      glTexImage1D(GL_TEXTURE_1D, i + 1, m_components, mipmap.get_width(), 0,
...
bool CDDSImage::upload_texture2D(unsigned int imageIndex, GLenum target) {
  ...
  if (is_compressed()) {
    ...
    for (unsigned int i = 0; i < image.get_num_mipmaps(); i++) {
      ...
      glCompressedTexImage2DARB(target, i + 1, m_format, mipmap.get_width(),
    ...
    glTexImage2D(target, 0, m_components, image.get_width(), image.get_height(),
...
inline unsigned int CDDSImage::size_rgb(unsigned int width,
                                        unsigned int height) {
...
inline void CDDSImage::swap_endian(void *val) {
  ...
  *ival = ((*ival >> 24) & 0x000000ff) | ((*ival >> 8) & 0x0000ff00) |
          ((*ival << 8) & 0x00ff0000) | ((*ival << 24) & 0xff000000);
#endif
}
...
CTexture::CTexture(unsigned int w, unsigned int h, unsigned int d,
                   unsigned int imgsize, const unsigned char *pixels)
...
void CTexture::create(unsigned int w, unsigned int h, unsigned int d,
                      unsigned int imgsize, const unsigned char *pixels) {
...
CSurface::CSurface(unsigned int w, unsigned int h, unsigned int d,
                   unsigned int imgsize, const unsigned char *pixels)
    : m_width(0), m_height(0), m_depth(0), m_size(0), m_pixels(NULL) {
...
///////////////////////////////////////////////////////////////////////////////
// creates an empty image
void CSurface::create(unsigned int w, unsigned int h, unsigned int d,
                      unsigned int imgsize, const unsigned char *pixels) {
  assert(w != 0);
  assert(h != 0);
  assert(d != 0);
