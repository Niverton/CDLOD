...
TerrainRenderer::TerrainRenderer(float planetRadius)
    : mPlanetRadius(planetRadius), mpQuadTree(NULL), mpTerrainUniformBuffer(0),
      mpAtmosphereUniformBuffer(0), mTerrainDimensionLocation(0) {
  int resolution = 64;
...
void TerrainRenderer::Initialize() {
  ...
  GLuint perFrameUniformIndex =
      glGetUniformBlockIndex(mpTerrainProgram, "UniformPerFrameBlock");
  glUniformBlockBinding(mpTerrainProgram, perFrameUniformIndex, 1);

  GLuint perObjectUniformIndex =
      glGetUniformBlockIndex(mpTerrainProgram, "UniformPerObjectBlock");
  glUniformBlockBinding(mpTerrainProgram, perObjectUniformIndex, 2);

  // Initialize terrain uniform buffer
  GLuint perTerrainPatchIndex =
      glGetUniformBlockIndex(mpTerrainProgram, "TerrainUniformBuffer");
  glUniformBlockBinding(mpTerrainProgram, perTerrainPatchIndex, 3);

  GLuint atmosphereUniformIndex =
      glGetUniformBlockIndex(mpTerrainProgram, "UniformAtmosphereBlock");
  glUniformBlockBinding(mpTerrainProgram, atmosphereUniformIndex, 4);

  // mTerrainDimensionLocation = glGetUniformLocation(mpTerrainProgram,
  // "TerrainDimensions");

  glGenBuffers(1, &mpTerrainUniformBuffer);
  glBindBuffer(GL_UNIFORM_BUFFER, mpTerrainUniformBuffer);
  glBufferData(GL_UNIFORM_BUFFER, sizeof(TerrainUniformBuffer), NULL,
               GL_DYNAMIC_DRAW);
  ...
  atmosphereBlock.v3InvWavelength =
      vec3(1.0f / wavelengths[0], 1.0f / wavelengths[1], 1.0f / wavelengths[2]);
  ...
  glBufferData(GL_UNIFORM_BUFFER, sizeof(TerrainUniformBuffer),
               &atmosphereBlock, GL_STATIC_DRAW);
...
void TerrainRenderer::Update(const Camera &camera, float dt) {
  ...
  std::map<unsigned long long int, char> oldNodeIds;

  for (auto it = mpQuadTree->mCurrentRenderingNodes.begin();
       it != mpQuadTree->mCurrentRenderingNodes.end(); ++it) {
  ...
  for (auto it = mpQuadTree->mCurrentRenderingNodes.begin();
       it != currentNodesEnd; ++it)
    mpHeightmapManager->getHeightmap((*it)->getID(), (*it)->getOffset(),
                                     (*it)->getScale(), vec3(0.0f, 1.0f, 0.0f));
}

void TerrainRenderer::Render(const PerFrameBlock &perFrame,
                             GLuint perObjectBufferRef,
                             GLuint lastProgramReference) {
  glEnable(GL_BLEND);
  glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
  glUseProgram(mpTerrainProgram);

  mat4 terrainWorld =
      glm::scale(mat4(1.0f), vec3(1.0)); //, vec3(0.0f, -mPlanetRadius, 0.0f));
  ...
  // glUniform2i(mTerrainDimensionLocation, mpMesh->getResolution(),
  // mpMesh->getResolution());
  ...
  for (auto it = mpQuadTree->mCurrentRenderingNodes.begin();
    ...
    mpMesh->setCurrentTexture(mpHeightmapManager->getHeightmap(
        (*it)->getID(), (*it)->getOffset(), (*it)->getScale(),
        vec3(0.0f, 1.0f, 0.0f)));
  ...
  // mpGrassRenderer->Render(perFrame, perObjectBufferRef,
  // lastProgramReference);  std::cout << "Meshes rendered: " <<
  // mpQuadTree->mCurrentRenderingNodes.size() << std::endl;
