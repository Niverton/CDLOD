...
// cannot directly read this struct, since it would need to be tightly packed
// for that to work
/*	struct TGAHeader
        {
                unsigned char  	ucIdent;		// size of ID
   field that follows 18 byte header (0 usually) unsigned char
   ucColorMapType;	// type of colour map 0=none, 1=has palette unsigned
   char  	ucImageType;    // type of image
   0=none,1=indexed,2=rgb,3=grey,+8=rle packed

                short 			colourmapstart;	// first
   colour map entry in palette
                short 			colourmaplength;// number of
   colours in palette unsigned char	colourmapbits;  // number of bits per
   palette entry 15,16,24,32

                short 			iXStart;  		// image
   x origin short 			iYStart;  		// image y
   origin
                short 			iWidth;   		// image
   width in pixels
                short 			iHeight;  		// image
   height in pixels
                unsigned char  	uiBits;			// image
   bits per pixel 8,16,24,32 unsigned char  	uiDescriptor;	// image
   descriptor bits (vh flip bits)
        };*/
...
bool IMAGE::LoadTGA(const char *szFile) {
  ...
  if ((m_iImageWidth <= 0) || (m_iImageHeight <= 0) ||
      ((m_iBytesPerPixel != 1) && (m_iBytesPerPixel != 3) &&
       (m_iBytesPerPixel != 4))) {
    fclose(pFile);
    return (false);
  }
...
void IMAGE::LoadUncompressedTGA(FILE *pFile) {
  ...
  for (int i = 0; i < iPixelCount; ++i) {
    ...
    // if it is a 24-Bit TGA (3 channels), the fourth channel stays at 255 all
    // the time, since the 4th value in ucBuffer is never overwritten
...
void IMAGE::LoadCompressedTGA(FILE *pFile) {
  ...
  do {
    ...
    if (ucChunkHeader < 128) {
      // If the header is < 128, it means it is the number of RAW color packets
      // minus 1 that follow the header add 1 to get number of following color
      // values
      ...
      for (int i = 0; i < (int)ucChunkHeader; ++i) {
        ...
        // if this is an 8-Bit TGA only, store the one channel in all four
        // channels if it is a 24-Bit TGA (3 channels), the fourth channel stays
        // at 255 all the time, since the 4th value in ucBuffer is never
        // overwritten
        if (m_iBytesPerPixel == 1) {
          ucBuffer[1] = ucBuffer[0];
          ucBuffer[2] = ucBuffer[0];
