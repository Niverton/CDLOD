...
ParticleSystem::ParticleSystem()
    : mpParticleTexture(NULL), mpParticleRenderProgram(0), mPerFrameBuffer(0),
      mParticleCount(0), mParticleBuffer(0), mPerParticleBuffer(0),
      mVertexArray(0), mComputeDtLocation(0), mParticleVelocityBuffer(0) {}
...
void ParticleSystem::Initialize() {
  ShaderInfo shaderInfo[] = {{GL_VERTEX_SHADER, "Shaders/particle_vs.glsl"},
  ...
  GLuint perFrameUniformIndex =
      glGetUniformBlockIndex(mpParticleRenderProgram, "UniformPerFrameBlock");
  glUniformBlockBinding(mpParticleRenderProgram, perFrameUniformIndex, 1);

  GLuint perParticleBlockIndex = glGetUniformBlockIndex(
      mpParticleRenderProgram, "UniformParticleSystemBlock");
  ...
  glBufferData(GL_UNIFORM_BUFFER, sizeof(PerParticleSystemBlock), &perParticle,
               GL_DYNAMIC_DRAW);
  ...
  // Compute shader
  ShaderInfo computeInfo[] = {{GL_COMPUTE_SHADER, "Shaders/particle_cs.glsl"},
                              {GL_NONE, NULL}};
  ...
  glBufferData(GL_UNIFORM_BUFFER, sizeof(AttractorBlock), NULL,
               GL_DYNAMIC_DRAW);
...
void ParticleSystem::InitializeParticleBuffer() {
  ...
  vec4 *particlePositionArray = (vec4 *)glMapBufferRange(
      GL_ARRAY_BUFFER, 0, PARTICLE_COUNT * sizeof(vec4),
      GL_MAP_WRITE_BIT | GL_MAP_INVALIDATE_BUFFER_BIT);

  for (int i = 0; i < PARTICLE_COUNT; i++) {
    particlePositionArray[i] = vec4((rand() / (float)RAND_MAX) * 50.0f,
                                    (rand() / (float)RAND_MAX) * 50.0f,
                                    (rand() / (float)RAND_MAX) * 50.0f, 1.0f);
  }
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(vec4) * PARTICLE_COUNT, NULL,
               GL_DYNAMIC_COPY);

  vec4 *particleVelocityArray = (vec4 *)glMapBufferRange(
      GL_ARRAY_BUFFER, 0, PARTICLE_COUNT * sizeof(vec4),
      GL_MAP_WRITE_BIT | GL_MAP_INVALIDATE_BUFFER_BIT);

  for (int i = 0; i < PARTICLE_COUNT; i++) {
    particleVelocityArray[i] =
        glm::normalize(vec4(rand() / (float)RAND_MAX - 0.5f,
...
void ParticleSystem::Update(float dt) {
  ...
  AttractorBlock *attractors = (AttractorBlock *)glMapBufferRange(
      GL_UNIFORM_BUFFER, 0, sizeof(AttractorBlock),
      GL_MAP_WRITE_BIT | GL_MAP_INVALIDATE_BUFFER_BIT);
  ...
  glBindImageTexture(0, mParticleVelocityTexBuffer, 0, GL_FALSE, 0,
                     GL_READ_WRITE, GL_RGBA32F);
  glBindImageTexture(1, mParticlePositionTexBuffer, 0, GL_FALSE, 0,
                     GL_READ_WRITE, GL_RGBA32F);
...
void ParticleSystem::Draw() {
  ...
  glBindBuffer(GL_UNIFORM_BUFFER, mPerParticleBuffer);
  glBufferData(GL_UNIFORM_BUFFER, sizeof(PerParticleSystemBlock), &perParticle,
  GL_DYNAMIC_DRAW); glBindBufferBase(GL_UNIFORM_BUFFER, 3, mPerParticleBuffer);
*/
