...
AtmosphereRenderer::AtmosphereRenderer()
    : mpAtmosphereSphere(NULL), mpWorldSphere(NULL),
      mpSkyFromAtmosphereProgram(0), mpSkyFromSpaceProgram(0),
...
void AtmosphereRenderer::Initialize() {
  ...
  perFrameUniformIndex = glGetUniformBlockIndex(mpSkyFromAtmosphereProgram,
                                                "UniformPerFrameBlock");
  glUniformBlockBinding(mpSkyFromAtmosphereProgram, perFrameUniformIndex, 1);

  perObjectUniformIndex = glGetUniformBlockIndex(mpSkyFromAtmosphereProgram,
                                                 "UniformPerObjectBlock");
  glUniformBlockBinding(mpSkyFromAtmosphereProgram, perObjectUniformIndex, 2);

  perFrameUniformIndex =
      glGetUniformBlockIndex(mpSkyFromSpaceProgram, "UniformPerFrameBlock");
  glUniformBlockBinding(mpSkyFromSpaceProgram, perFrameUniformIndex, 1);

  perObjectUniformIndex =
      glGetUniformBlockIndex(mpSkyFromSpaceProgram, "UniformPerObjectBlock");
...
void AtmosphereRenderer::InitializeAtmosphereUniform() {
  ...
  GLuint perAtmosphereBlockIndex = glGetUniformBlockIndex(
      mpSkyFromAtmosphereProgram, "UniformAtmosphereBlock");
  glUniformBlockBinding(mpSkyFromAtmosphereProgram, perAtmosphereBlockIndex, 3);

  perAtmosphereBlockIndex =
      glGetUniformBlockIndex(mpSkyFromSpaceProgram, "UniformAtmosphereBlock");
  ...
  glBufferData(GL_UNIFORM_BUFFER, sizeof(PerAtmosphereBlock), &atmosphereBlock,
               GL_STATIC_COPY);
  glBindBufferBase(GL_UNIFORM_BUFFER, 3, mpAtmosphereUniformBuffer);
}

void AtmosphereRenderer::Render(const PerFrameBlock &perFrame,
  ...
  mat4 atmosphereWorld =
      glm::translate(mat4(1.0f), vec3(0.0f, -worldScale, 0.0f));
  ...
  if (glm::length(perFrame.ViewPosition + vec3(0.0f, worldScale, 0.0f)) <
      atmosphereScale)
    glUseProgram(mpSkyFromAtmosphereProgram);
  else
