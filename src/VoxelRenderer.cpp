...
VoxelRenderer::VoxelRenderer() : mVoxelProgram(0) {
...
void VoxelRenderer::Initialize() {
  ...
  GLuint perFrameUniformIndex =
      glGetUniformBlockIndex(mVoxelProgram, "UniformPerFrameBlock");
  glUniformBlockBinding(mVoxelProgram, perFrameUniformIndex, 1);

  GLuint perObjectUniformIndex =
      glGetUniformBlockIndex(mVoxelProgram, "UniformPerObjectBlock");
  glUniformBlockBinding(mVoxelProgram, perObjectUniformIndex, 2);

  /*for (int i = 0; i < 30000; i++)
  {
          mpChunk1->vertexArray[rand() % mpChunk1->getWidth()][rand() %
  mpChunk1->getHeight()][rand() % mpChunk1->getDepth()].enabled = false;
  }*/
...
void VoxelRenderer::Render(const PerFrameBlock &perFrame,
  ...
  glBufferData(GL_UNIFORM_BUFFER, sizeof(PerObjectBlock), &perObject,
               GL_DYNAMIC_DRAW);
