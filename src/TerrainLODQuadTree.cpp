...
TerrainLODQuadTree::TerrainLODQuadTree(int maxLOD, float mostDetailedDistance,
                                       vec3 surfaceNormal, float planetRadius)
    : mMaxLOD(maxLOD), mMostDetailedDistance(mostDetailedDistance),
      mSurfaceNormal(surfaceNormal), mPlanetRadius(planetRadius) {
  assert(maxLOD < 28);
...
vec3 TerrainLODQuadTree::projectOntoSphere(const vec3 &pos) const {
  ...
  result.x = pos.x * sqrtf(1.0f - (posSqr.y / 2.0f) - (posSqr.z / 2.0f) +
                           ((posSqr.y * posSqr.z) / 3.0f));
  result.y = pos.y * sqrtf(1.0f - (posSqr.z / 2.0f) - (posSqr.x / 2.0f) +
                           ((posSqr.z * posSqr.x) / 3.0f));
  result.z = pos.z * sqrtf(1.0f - (posSqr.x / 2.0f) - (posSqr.y / 2.0f) +
                           ((posSqr.x * posSqr.y) / 3.0f));
...
dvec3 TerrainLODQuadTree::projectOntoSphere(const dvec3 &pos) const {
  ...
  result.x = pos.x * sqrt(1.0 - (posSqr.y / 2.0) - (posSqr.z / 2.0) +
                          ((posSqr.y * posSqr.z) / 3.0));
  result.y = pos.y * sqrt(1.0 - (posSqr.z / 2.0) - (posSqr.x / 2.0) +
                          ((posSqr.z * posSqr.x) / 3.0));
  result.z = pos.z * sqrt(1.0 - (posSqr.x / 2.0) - (posSqr.y / 2.0) +
                          ((posSqr.x * posSqr.y) / 3.0));
...
void TerrainLODQuadTree::Update(const Camera &camera) {
  ...
  for (auto it = mCurrentRenderingNodes.begin();
       it != mCurrentRenderingNodes.end(); ++it) {
  ...
  selectNodes(dvec3(camera.getPosition()) - dvec3(0.0f, -mPlanetRadius, 0.0f),
              dvec3(0.0f, 1.0f, 0.0f), 1.0f, 0, 0);
...
float TerrainLODQuadTree::calculateDistanceToCamera(const dvec3 &cameraPosWorld,
                                                    const dvec3 &planeCoord) {
  vec3 spherePosition = vec3(projectOntoSphere(planeCoord));

  vec3 dCameraPosition =
      vec3(cameraPosWorld.x, cameraPosWorld.y, cameraPosWorld.z);
  vec3 worldPosition =
      vec3(spherePosition.x, spherePosition.y, spherePosition.z) *
      mPlanetRadius;
...
void TerrainLODQuadTree::selectNodes(const dvec3 &cameraPosition,
                                     const dvec3 &position, double scale,
                                     int LODLevel, unsigned long long int currentId) {
  ...
  if (LODLevel < mMaxLOD - 1) {
    ...
    distanceToCamera =
        calculateDistanceToCamera(cameraPosition, nextNodePosition);
    ...
    if (distanceToCamera < mpLODDistanceArray[LODLevel]) {
      ...
      selectNodes(cameraPosition, nextNodePosition, halfScale, LODLevel + 1,
                  nextId);
  ...
  if (newNode->renderUL || newNode->renderUR || newNode->renderBL ||
      newNode->renderBR) {
    newNode->mOffset = dvec2(position.x, position.z);
    newNode->mScale = scale;
    newNode->mTransformMatrix =
        glm::translate(mat4(1.0f), vec3(position) - vec3(0.0f, 1.0f, 0.0f)) *
        glm::scale(mat4(1.0f), vec3((float)scale, 1.0f, (float)scale));
...
TerrainLODQuadTree::TerrainNode::TerrainNode()
    : mTransformMatrix(1.0f), mId(0), mOffset(dvec2(0.0)), mScale(1.0) {
  resetRenderParams();
}
