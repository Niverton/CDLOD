...
// Alternate shader file parser that treats '#include <>' and '#include ""' like
// include in C++
static GLchar *
ReadShaderInclude(const char *fileName,
                  std::vector<std::string> *loadedFileList = NULL) {
  ...
  while (std::getline(inFile, currentLine)) {
    ...
    if (subString == "#include <" || subString == "#include \"") {
      std::string includePath =
          currentLine.substr(10, currentLine.length() - 11);
      bool keyFound = false;

      // Forcing #pragma once emulation
      for (auto it = loadedFileList->begin(); it != loadedFileList->end();
           ++it) {
        if ((*it) == includePath) {
          keyFound = true;
          break;
      ...
      if (!keyFound) {
        ...
        const char *includeSource =
            ReadShaderInclude(includePath.c_str(), loadedFileList);
