...
ChunkMesh::ChunkMesh() : pSourceChunk(NULL), mTriangleCount(0) {}

ChunkMesh::~ChunkMesh() { glDeleteBuffers(1, &mpVertexBuffer); }

void ChunkMesh::Initialize() { glGenBuffers(1, &mpVertexBuffer); }
...
void ChunkMesh::rebuildChunk() {
  if (pSourceChunk) {
      ...
      for (int y = 0; y < height; y++) {
        for (int z = 0; z < depth; z++) {
          if (pSourceChunk->vertexArray[x][y][z].enabled) {
            ...
            addVertices(glm::ivec3(x, y, z), topActive, bottomActive,
                        frontActive, backActive, leftActive, rightActive,
                        vertices);
          }
        }
      }
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(VoxelVertex) * vertices.size(),
...
void ChunkMesh::addVertices(const glm::ivec3 &position, bool topActive,
                            bool bottomActive, bool frontActive,
                            bool backActive, bool leftActive, bool rightActive,
  ...
  if (topActive) {
    vertOut.push_back(
        VoxelVertex(basePosition + vec3(-hw, hh, -hd), vec3(0.0f, 1.0f, 0.0f)));
    vertOut.push_back(
        VoxelVertex(basePosition + vec3(-hw, hh, hd), vec3(0.0f, 1.0f, 0.0f)));
    vertOut.push_back(
        VoxelVertex(basePosition + vec3(hw, hh, -hd), vec3(0.0f, 1.0f, 0.0f)));
  ...
  if (bottomActive) {
    vertOut.push_back(VoxelVertex(basePosition + vec3(-hw, -hh, -hd),
                                  vec3(0.0f, -1.0f, 0.0f)));
    vertOut.push_back(VoxelVertex(basePosition + vec3(hw, -hh, -hd),
                                  vec3(0.0f, -1.0f, 0.0f)));
    vertOut.push_back(VoxelVertex(basePosition + vec3(-hw, -hh, hd),
  ...
  if (frontActive) {
    vertOut.push_back(
        VoxelVertex(basePosition + vec3(-hw, hh, hd), vec3(0.0f, 0.0f, 1.0f)));
    vertOut.push_back(
        VoxelVertex(basePosition + vec3(hw, -hh, hd), vec3(0.0f, 0.0f, 1.0f)));
    vertOut.push_back(
        VoxelVertex(basePosition + vec3(hw, hh, hd), vec3(0.0f, 0.0f, 1.0f)));
  ...
  if (rightActive) {
    vertOut.push_back(
        VoxelVertex(basePosition + vec3(hw, hh, -hd), vec3(1.0f, 0.0f, 0.0f)));
    vertOut.push_back(
        VoxelVertex(basePosition + vec3(hw, -hh, hd), vec3(1.0f, 0.0f, 0.0f)));
    vertOut.push_back(
        VoxelVertex(basePosition + vec3(hw, -hh, -hd), vec3(1.0f, 0.0f, 0.0f)));
