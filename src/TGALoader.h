...
public:
  ...
  //! Loads a TGA. Can be 8, 24 or 32 Bits per pixel, uncompressed or (RLE)
  //! compressed. Returns false, if the TGA could not be loaded.
  ...
  /*! ATTENTION:
          ...
          This allows faster load-times (TGAs are stored this way natively) and
     faster upload-times to OpenGL. Use GL_BGRA as "format", when uploading the
     data to OpenGL.
  */
  const unsigned char *getPixel(int x, int y) const {
    return (&m_Pixels[(y * m_iImageWidth + x) * 4]);
  }

  //! Returns the raw array, as needed when uploading the image to OpenGL.
  /*! Upload the image to OpenGL like this:
          IMAGE Image;
          gluBuild2DMipmaps (GL_TEXTURE_2D, 4, Image.getWidth (),
     Image.getHeight (), GL_BGRA, GL_UNSIGNED_BYTE, Image.getDataForOpenGL  ());
  */
