...
COLLADALoader::COLLADALoader(const char *filePath)
    : mFilePath(filePath), mMeshSources(), mpRootNode(NULL) {}

COLLADALoader::~COLLADALoader() { clear(); }
...
void COLLADALoader::loadDocument() {
  FILE *modelFile = fopen(mFilePath.c_str(), "rb");
...
void COLLADALoader::loadAssetParameters() {
  XMLElement *assetElement =
      mSceneDocument.FirstChildElement()->FirstChildElement("asset");
  assert(assetElement);
  ...
  const char *axisString =
      assetElement->FirstChildElement("up_axis")->GetText();
...
void COLLADALoader::readMeshSourceElement(XMLElement *element,
                                          MeshSource *storage) {
  XMLElement *floatArray = element->FirstChildElement("float_array");
  XMLElement *accessor = element->FirstChildElement("technique_common")
...
void COLLADALoader::readFloatArrayElement(XMLElement *element,
...
void COLLADALoader::readIntArrayElement(XMLElement *element,
                                        vector<int> &storage,
                                        int expectedSize) {
  storage.clear();
...
void COLLADALoader::associateAccessors(XMLElement *primitiveElement,
                                       MeshAccessor *sourceArray) {
  XMLElement *vertexElement =
      primitiveElement->PreviousSiblingElement("vertices")
  ...
  while (inputElement) {
    ...
    if (*(source - 1) != '#') {
      cout << "Invalid URL format. Expected #, Received URL: " << (source - 1)
           << endl;
      return;
    }
    ...
    if (strcmp(semantic, "NORMAL") == 0)
      ...
      arrayIndex = 2;
             ...
             strcmp(semantic, "TEXTANGENT") ==
                 0) // Not sure about the practical difference at the moment
      arrayIndex = 3;
...
void COLLADALoader::loadModels() {
  XMLElement *geometryLibElement =
      mSceneDocument.FirstChildElement()->FirstChildElement(
          "library_geometries");
  ...
  while (currentGeometryElement != NULL) {
    ...
    currentGeometryElement =
        currentGeometryElement->NextSiblingElement("geometry");
  }
}
...
void COLLADALoader::constructModelGeometry(XMLElement *geometryElement) {
  if (mModels.find(geometryElement->Attribute("id")) != mModels.end() ||
      ...
      geometryElement->FirstChildElement("mesh")->FirstChildElement("source") ==
          NULL) {
    /*if (destination != NULL)
  ...
  string vertexSourceId(meshElement->FirstChildElement("vertices")
                            ->FirstChildElement("input")
                            ->Attribute("source") +
                        1);
  vertexSource = mMeshSources[vertexSourceId];

  // Since there can be a mix of multiple triangles and polylist elements that
  ...
  while (primitiveElement != NULL) {
    ...
    if (primitiveElement->Attribute("material") != NULL)
      subMesh->effectId =
          mMaterials[primitiveElement->Attribute("material")].effectId;

    primitiveCount = primitiveElement->IntAttribute("count");

    // Calculate final vertex count
    if (primitiveType == PolyList) {
      readIntArrayElement(primitiveElement->FirstChildElement("vcount"),
                          polyListvCount, primitiveCount);
    ...
    MeshAccessor accessorArray[5]; // Corresponds to the source arrays for
                                   // POSITION, NORMAL, TEXCOORD, TANGENT, and
                                   // BINORMAL
    ...
    // Read the index list
    readIntArrayElement(primitiveElement->FirstChildElement("p"), pList,
                        vertexCount);
    ...
    for (int i = 0; i < primitiveCount; i++) {
      if (primitiveType == PolyList) {
        ...
        if (pCount != 3)
          cout << "Non-triangle primitives are unsupported at this time."
               << endl;
      }
      ...
      for (int p = 0; p < pCount; p++) {
        ...
        vert.Position =
            vec3(accessorArray[0]
                         .source
                         ->data[pList[currentIndex + accessorArray[0].offset] *
                                accessorArray[0].source->stride] *
                     mScalingFactor,
                 accessorArray[0]
                         .source
                         ->data[pList[currentIndex + accessorArray[0].offset] *
                 ...
                 accessorArray[0]
                         ...
                         ->data[pList[currentIndex + accessorArray[0].offset] *
                                    accessorArray[0].source->stride +
                                zOffset] *
                     mScalingFactor * zMultiplier);

        if (accessorArray[1].source != NULL)
          vert.Normal = vec3(
              accessorArray[1]
                  .source->data[pList[currentIndex + accessorArray[1].offset] *
                                accessorArray[1].source->stride],
              accessorArray[1]
                  .source->data[pList[currentIndex + accessorArray[1].offset] *
                                    accessorArray[1].source->stride +
                                yOffset],
              accessorArray[1]
                      .source
                      ->data[pList[currentIndex + accessorArray[1].offset] *
                                 accessorArray[1].source->stride +
                             zOffset] *
                  zMultiplier);
        ...
        if (accessorArray[2].source != NULL)
          vert.TexCoord = vec2(
              accessorArray[2]
                  .source->data[pList[currentIndex + accessorArray[2].offset] *
                                accessorArray[2].source->stride],
              accessorArray[2]
                  .source->data[pList[currentIndex + accessorArray[2].offset] *
                                    accessorArray[2].source->stride +
                                1]);
        ...
        if (accessorArray[3].source != NULL)
          vert.Tangent = vec3(
              accessorArray[3]
                  .source->data[pList[currentIndex + accessorArray[3].offset] *
                                accessorArray[3].source->stride],
              accessorArray[3]
                  .source->data[pList[currentIndex + accessorArray[3].offset] *
                                    accessorArray[3].source->stride +
                                yOffset],
              accessorArray[3]
                      .source
                      ->data[pList[currentIndex + accessorArray[3].offset] *
                                 accessorArray[3].source->stride +
                             zOffset] *
                  zMultiplier);
        ...
        if (accessorArray[4].source != NULL)
          vert.Bitangent = vec3(
              accessorArray[4]
                  .source->data[pList[currentIndex + accessorArray[4].offset] *
                                accessorArray[4].source->stride],
              accessorArray[4]
                  .source->data[pList[currentIndex + accessorArray[4].offset] *
                                    accessorArray[4].source->stride +
                                yOffset],
              accessorArray[4]
                      .source
                      ->data[pList[currentIndex + accessorArray[4].offset] *
                                 accessorArray[4].source->stride +
                             zOffset] *
                  zMultiplier);
        ...
        /*	else
                        subMesh->mesh.Indices.push_back((i + 1) * pCount - p -
           1);*/
      }
...
void COLLADALoader::loadEffects() {
  ...
  if (mSceneDocument.FirstChildElement()->FirstChildElement("library_effects"))
    currentEffectElement = mSceneDocument.FirstChildElement()
                               ->FirstChildElement("library_effects")
  ...
  while (currentEffectElement != NULL) {
    ...
    XMLElement *profileElement =
        currentEffectElement->FirstChildElement("profile_COMMON");

    if (profileElement == NULL) {
      cout << "Only profile_COMMON is supported at the moment. Effect: " << id
           << " uses " << currentEffectElement->FirstChildElement()->Value()
           << endl;
      throw -
          1; // Probably going to crash because effect will be missing anyways
    ...
    XMLElement *techinqueElement =
        profileElement->FirstChildElement("technique");
    ...
    // Go through different types of lighting parameters to determine correct
    // type
    ...
    if (attributeElement &&
        attributeElement->FirstChildElement("color") != NULL)
      readColor(attributeElement->FirstChildElement("color"),
                currentEffect->emission);
    else if (attributeElement &&
             attributeElement->FirstChildElement("texture") != NULL) {
      surfaceId =
          samplerMap[attributeElement->FirstChildElement("texture")->Attribute(
                         "texture")]
              .surfaceId;
    ...
    if (attributeElement &&
        attributeElement->FirstChildElement("float") != NULL) {
      currentEffect->specularPower =
          (float)atof(attributeElement->FirstChildElement("float")->GetText());
    }
    ...
    if (extraElement && extraElement->FirstChildElement("technique") &&
        extraElement->FirstChildElement("technique")
            ->FirstChildElement("bump")) {
      bumpMapElement = extraElement->FirstChildElement("technique")
                           ->FirstChildElement("bump");
    }

    if (bumpMapElement && bumpMapElement->FirstChildElement("texture")) {
      surfaceId =
          samplerMap[bumpMapElement->FirstChildElement("texture")->Attribute(
                         "texture")]
              .surfaceId;
      imageId = surfaceMap[surfaceId].imageId;
...
void COLLADALoader::loadNewParamElements(XMLElement *profileElement,
                                         map<string, Surface> &surfaceMap,
                                         map<string, Sampler> &samplerMap) {
  ...
  while (paramElement != NULL) {
    ...
    if (paramElement->FirstChildElement("surface") != NULL) {
      Surface newSurface = {paramElement->FirstChildElement("surface")
                                ->FirstChildElement("init_from")
                                ->GetText()};
      surfaceMap[id] = newSurface;
    } else if (paramElement->FirstChildElement("sampler2D") != NULL) {
      Sampler newSampler = {paramElement->FirstChildElement("sampler2D")
...
void COLLADALoader::loadMaterials() {
  ...
  while (currentMaterialElement) {
    ...
    XMLElement *instanceEffectElement =
        currentMaterialElement->FirstChildElement("instance_effect");
    ...
    currentMaterialElement =
        currentMaterialElement->NextSiblingElement("material");
  }
}

void COLLADALoader::loadImages() {
  XMLElement *imageLibElement =
      mSceneDocument.FirstChildElement()->FirstChildElement("library_images");
  ...
  while (currentImageElement != NULL) {
    ...
    if (currentImageElement->FirstChildElement("data") != NULL) {
      cout << "Image embedding is not currently supported. Image element: "
           << id << endl;
      throw - 1;
    }

    mImages[id] =
        Image(currentImageElement->FirstChildElement("init_from")->GetText());
...
void COLLADALoader::loadSceneNodes() {
  ...
  XMLElement *rootScene = mSceneDocument.FirstChildElement()->FirstChildElement(
...
void COLLADALoader::loadNode(SceneNode *parent, XMLElement *nodeElement) {
  XMLElement *geometryElement =
      nodeElement->FirstChildElement("instance_geometry");
  ...
  if (geometryElement) {
    ...
    if (geometryElement->FirstChildElement("bind_material") &&
        geometryElement->FirstChildElement("bind_material")
            ->FirstChildElement("technique_common")) {
      materialElement = geometryElement->FirstChildElement("bind_material")
                            ->FirstChildElement("technique_common");
...
void COLLADALoader::generateTangents() {
  for (auto modelIt = mModels.begin(); modelIt != mModels.end(); ++modelIt) {
    ...
    for (auto meshIt = modelIt->second->subMeshes.begin();
         meshIt != modelIt->second->subMeshes.end(); ++meshIt) {
      if ((*meshIt)->mesh.Vertices.size() > 0 &&
          ((*meshIt)->mesh.Vertices[0].Tangent.length() == 0.0f &&
           (*meshIt)->mesh.Vertices[0].Bitangent.length() == 0.0f))
        continue;
      ...
      for (unsigned i = 0; i < (*meshIt)->mesh.Indices.size() - 2;) {
        ...
        if (denominator != 0.0f) {
          ...
          tangent.x =
              (vVector[1] * tangentVector1.x - vVector[0] * tangentVector2[0]) *
              denominator;
          tangent.y = (vVector[1] * tangentVector1[1] -
                       vVector[0] * tangentVector2[1]) *
                      denominator;
          tangent.z = (vVector[1] * tangentVector1[2] -
                       vVector[0] * tangentVector2[2]) *
                      denominator;

          binormal.x = (uVector[0] * tangentVector2[0] -
                        uVector[1] * tangentVector1[0]) *
                       denominator;
          binormal.y = (uVector[0] * tangentVector2[1] -
                        uVector[1] * tangentVector1[1]) *
                       denominator;
          binormal.z = (uVector[0] * tangentVector2[2] -
                        uVector[1] * tangentVector1[2]) *
                       denominator;

          glm::normalize(tangent);
          glm::normalize(binormal);

          (*meshIt)->mesh.Vertices[(*meshIt)->mesh.Indices[i]].Tangent =
              tangent;
          (*meshIt)->mesh.Vertices[(*meshIt)->mesh.Indices[i]].Bitangent =
              binormal;
          (*meshIt)->mesh.Vertices[(*meshIt)->mesh.Indices[i + 1]].Tangent =
              tangent;
          (*meshIt)->mesh.Vertices[(*meshIt)->mesh.Indices[i + 1]].Bitangent =
              binormal;
          (*meshIt)->mesh.Vertices[(*meshIt)->mesh.Indices[i + 1]].Tangent =
              tangent;
          (*meshIt)->mesh.Vertices[(*meshIt)->mesh.Indices[i + 1]].Bitangent =
