...
void VoxelTreeGenerator::generateSegments(const vec3 &startPoint,
                                          const vec3 &direction, float length,
                                          int itCount) {
  if (itCount < 7) {
    LineSegment segment;
    ...
    vec3 xRotationVec =
        glm::normalize(glm::cross(direction, vec3(1.0f, 0.0f, 0.0f)));
    vec3 yRotationVec =
        glm::normalize(glm::cross(direction, vec3(0.0f, 1.0f, 0.0f)));
    vec3 zRotationVec =
        glm::normalize(glm::cross(direction, vec3(0.0f, 0.0f, 1.0f)));
...
void VoxelTreeGenerator::rasterizePoint(const vec3 &point) {
  if (point.x > 0.0f && point.x < mpTargetChunk->getWidth() - 1.0f &&
    ...
    mpTargetChunk->vertexArray[(int)point.x][(int)point.y][(int)point.z]
        .enabled = true;
  }
}
