...
GLApp::GLApp(GLFWwindow *window)
    : pWindow(window), mCurrentRotation(0.0f), mClientWidth(1280.0f),
      mClientHeight(720.0f),
      mCamera(window, vec3(-20.0f, 10.0f, -20.0f), vec3(0.0f, 1.0f, 0.0f),
              vec3(0.0f, 1.0f, 0.0f)),
      mWireFrame(false), mCullBackfaces(false), mRotateModels(true),
      mpParticleSystem(NULL), mpAtmosphereRenderer(NULL), mpTerrain(NULL),
...
void GLApp::Initialize() {
  ...
  ShaderInfo shaderInfo[] = {{GL_VERTEX_SHADER, "Shaders/basic_vs.glsl"},
  ...
  perObjectUniformIndex =
      glGetUniformBlockIndex(renderProgram, "UniformPerObjectBlock");
  perFrameUniformIndex =
      glGetUniformBlockIndex(renderProgram, "UniformPerFrameBlock");
  ...
  glBufferData(GL_UNIFORM_BUFFER, sizeof(PerFrameBlock), &perFrame,
               GL_DYNAMIC_DRAW);
  ...
  glBufferData(GL_UNIFORM_BUFFER, sizeof(PerObjectBlock), &objectBlock,
               GL_DYNAMIC_DRAW);
  ...
  GameObjectPlaceHolder *ground =
      new GameObjectPlaceHolder(vec3(0.0f, 0.0f, 0.0f));
  ...
  ground->mpDiffuseTexture =
      TextureManager::getInstance().getTexture("rock.dds");
...
void GLApp::InitializeModels(int argc, char *argv[]) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    GameObjectPlaceHolder *rootObject =
        new GameObjectPlaceHolder(vec3(3.5f * (i - 1), 3.0f, 0.0f));
...
void GLApp::InitializeFromColladaNode(COLLADALoader &loader,
                                      GameObjectPlaceHolder *gameObjectNode,
                                      collada::SceneNode *colladaNode) {
  TextureManager &textureManager = TextureManager::getInstance();
  ...
  if (colladaNode->effectId != "") {
    ...
    if (effect->diffuseTextureId != "") {
      ...
      gameObjectNode->mpDiffuseTexture =
          textureManager.getTexture(effect->diffuseTextureId.c_str());
    }
    if (effect->bumpMapId != "") {
      textureManager.loadTexture(effect->bumpMapId.c_str());
      gameObjectNode->mpBumpMap =
          textureManager.getTexture(effect->bumpMapId.c_str());
    }
    if (effect->specularMapId != "") {
      textureManager.loadTexture(effect->specularMapId.c_str());
      gameObjectNode->mpSpecularMap =
          textureManager.getTexture(effect->specularMapId.c_str());
    }
    if (effect->emissiveMapId != "") {
      textureManager.loadTexture(effect->emissiveMapId.c_str());
      gameObjectNode->mpEmissiveMap =
          textureManager.getTexture(effect->emissiveMapId.c_str());
    }
  }

  // Load submeshes
  if (colladaNode->model) {
    for (auto it = colladaNode->model->subMeshes.begin();
         it != colladaNode->model->subMeshes.end(); ++it) {
      ...
      if ((*it)->effectId != "") {
        ...
        material->Specular = effect->specular;
        material->Specular.w =
            effect->specularPower; // Pack specular power into w
        material->Emissive = effect->emission;

        if (effect->diffuseTextureId != "") {
          textureManager.loadTexture(effect->diffuseTextureId.c_str());
          currentMesh->mpDiffuseTexture =
              textureManager.getTexture(effect->diffuseTextureId.c_str());
        }
        if (effect->bumpMapId != "") {
          textureManager.loadTexture(effect->bumpMapId.c_str());
          currentMesh->mpBumpMap =
              textureManager.getTexture(effect->bumpMapId.c_str());
        }
        if (effect->specularMapId != "") {
          textureManager.loadTexture(effect->specularMapId.c_str());
          currentMesh->mpSpecularMap =
              textureManager.getTexture(effect->specularMapId.c_str());
        }
        if (effect->emissiveMapId != "") {
          textureManager.loadTexture(effect->emissiveMapId.c_str());
          currentMesh->mpEmissiveMap =
              textureManager.getTexture(effect->emissiveMapId.c_str());
        }
      }
    }
  ...
  for (auto it = colladaNode->children.begin();
       it != colladaNode->children.end(); ++it) {
    GameObjectPlaceHolder *newGameObject =
        new GameObjectPlaceHolder(vec3(0.0f));
...
void GLApp::OnMouseMove(float x, float y) { mCamera.OnMouseMove(x, y); }
...
void GLApp::Update() {
  ...
  if (glfwGetKey(pWindow, GLFW_KEY_Z) == GLFW_PRESS) {
    mSunDirection = glm::normalize(
        (mat3)glm::rotate(mat4(1.0f), (float)(10.1f * mTimer.DeltaTime()),
                          glm::normalize(vec3(1.0f, 0.2f, 0.0f))) *
        mSunDirection);
  }
...
void GLApp::UpdateFrameCounter(double dt) {
  ...
  if (totalTime >= 1.0f) {
    ...
    title << "Frame time: " << averageime * 1000.0f
          << "ms   FPS: " << 1.0f / averageime
          << " Velocity: " << mCamera.getVelocity() << "m/s";
...
void GLApp::Render() {
  ...
  spotLight.Direction =
      glm::normalize(vec3(0.0f, 0.0f, 0.0f) - spotLight.Position);
  ...
  perFrame.ViewPosition = mCamera.getPosition();
  perFrame.ViewPositionD = mCamera.getPosition();
  ...
  for (auto it = mLoadedModels.begin(); it != mLoadedModels.end(); ++it) {
    ...
    if (it != mLoadedModels.begin())
      world = glm::translate(mat4(1.0f), (*it)->mPosition) *
              glm::scale(mat4(1.0f), vec3(1.0f, 1.0f, 1.0f)) *
              glm::rotate(mat4(1.0f), mCurrentRotation, vec3(0.0f, 1.0f, 0.0f));
