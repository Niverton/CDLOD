...
TerrainMeshCompute::TerrainMeshCompute(int resolution, float scale)
    : mResolution(resolution), mScale(scale), mpVertexBuffer(0),
      mpVertexArray(0), mpUniformBuffer(0) {
  resetRenderParams();
...
void TerrainMeshCompute::Initialize() {
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(vec4) * vertices.size(), &vertices[0],
               GL_STATIC_DRAW);
  ...
  for (int i = 0; i < 4; i++) {
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(GLushort) * indices[i].size(),
                 &indices[i][0], GL_STATIC_DRAW);
  }
...
void TerrainMeshCompute::Generate(const dvec2 &offset, double scale,
  ...
  glBindImageTexture(0, mpVertexTexture, 0, GL_FALSE, 0, GL_READ_WRITE,
                     GL_RGBA32F);

  glDispatchCompute(4, 1, 1); // 256 threads for a total of 1024 vertices so we
                              // are expecting a 32x32 grid
...
void TerrainMeshCompute::generateMesh(
    int resolution, float scale, std::vector<vec4> &vertexOut,
  ...
  for (int x = 0; x < resolution; x++) {
    ...
    for (int z = 0; z < resolution; z++) {
      ...
      vertexOut.push_back(
          vec4(currentX, 0.5f * scale, -halfDepth + z * dz, 0.0f));
    }
  }
