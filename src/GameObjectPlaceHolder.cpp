...
GameObjectPlaceHolder::GameObjectPlaceHolder(const vec3 &position)
    : mPosition(position), mpDiffuseTexture(NULL), mpBumpMap(NULL),
      mpSpecularMap(NULL), mpEmissiveMap(NULL), mpMaterial(NULL),
...
void GameObjectPlaceHolder::Draw(const PerObjectBlock &parentBlock,
                                 GLuint bufferRef) {
