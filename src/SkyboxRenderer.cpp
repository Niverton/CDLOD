...
SkyboxRenderer::SkyboxRenderer()
    : mVertexBuffer(0), mIndexBuffer(0), mVertexArray(0), mIndexCount(0),
      mpSkyboxTexture(NULL), mpSkyboxRenderProgram(0) {}
...
void SkyboxRenderer::Initialize() {
  ...
  // Load shader
  ShaderInfo shaderInfo[] = {{GL_VERTEX_SHADER, "Shaders/skybox_vs.glsl"},
  ...
  GLuint perFrameUniformIndex =
      glGetUniformBlockIndex(mpSkyboxRenderProgram, "UniformPerFrameBlock");
  glUniformBlockBinding(mpSkyboxRenderProgram, perFrameUniformIndex, 1);

  GLuint perObjectUniformIndex =
      glGetUniformBlockIndex(mpSkyboxRenderProgram, "UniformPerObjectBlock");
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(SkyboxVert) * vertices.size(),
...
void SkyboxRenderer::generateSkybox(vector<SkyboxVert> &verticesOut,
                                    vector<GLuint> &indicesOut) {
  // front face
...
void SkyboxRenderer::Render(const PerFrameBlock &perFrame,
  ...
  mat4 skyWorld =
      mat4(1.0f) * glm::scale(mat4(1.0f), vec3(1000.0f, 1000.0f, 1000.0f));
  ...
  glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(SkyboxVert),
                        (char *)NULL);
